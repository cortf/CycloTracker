...
export function DataTable({ states, caption }: { states: StateEntry[]; caption: string }) {
  const rows = [...states].sort((a, b) => b.count - a.count || a.name.localeCompare(b.name));
  return (
    <table className="data-table">
      ...
      <thead>
        <tr>
          ...
          <th scope="col" style={{ textAlign: "right" }}>Per 100k</th>
      ...
      <tbody>
        {rows.map((s) => {
          ...
          return (
            <tr key={s.fips}>
              <th scope="row">{s.name} ({s.usps})</th>
              <td className="num">{noData ? "—" : fmtCount(s.count)}</td>
              <td className="num">{fmtRate(s.rate)}</td>
              <td className={noData ? "cls-no-data" : undefined}>{classificationText(s.classification)}</td>
